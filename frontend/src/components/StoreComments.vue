...
const isMerchant = computed(() => userStore.user?.role === ROLES.MERCHANT && userStore.user?.id === props.storeMerchantId)
const isCustomer = computed(() => userStore.user?.role === ROLES.CUSTOMER)
const canAddComment = computed(() => userStore.isLoggedIn && isCustomer.value)
...
<template>
  <div class="comments-section">
    ...
    <!-- 未登录提示 -->
    <div v-else-if="!userStore.isLoggedIn" class="login-tip">
      <p>请先登录后再发表评论</p>
    </div>

    <!-- 商家提示 -->
    <div v-else-if="isMerchant" class="merchant-tip">
      <p>商家不能对自己的店铺发表评论</p>
    </div>
