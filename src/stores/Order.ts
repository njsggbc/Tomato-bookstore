...
export const useOrderStore = defineStore('order', () => {
  ...
  const payOrder = async () => {
    ...
    try {
      ...
      params.append('totalAmount', currentOrder.value.totalAmount.toString());
      const response = await api.post(`/payments/pay`, params, {
      ...
      if (paymentUrl && typeof paymentUrl === 'string') {
        ...
        // 用户在新标签页中完成支付后，可以手动刷新此页面查看最新状态
      } else {
        throw new Error('未能获取有效的支付链接')
      }
    } catch (err: any) {
      error.value = err.response?.data?.msg || err.message || '发起支付失败，请重试'
    }
  }
